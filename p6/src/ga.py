...
class Individual_Grid(object):
    ...
    def generate_children(self, other):
        new_genome = copy.deepcopy(self.genome)
        other = copy.deepcopy(other.genome)
        ...
        for y in range(height):
            for x in range(left, right):
                ...
                if random.randint(1, 100) <= 10:
                    ...
                    copy_genome[y][x] = other[y][x] if other[y][x] != 'T' and other[y][x] != '|' else new_genome[y][x]
                else:
                    pass

        # do mutation; note we're returning a one-element tuple here
        return (Individual_Grid(self.mutate(copy_genome)))
...
def generate_successors(population):
    ...
    results = []
    # print(population)
    t_children = tournament_selection(population)
    r_children = roulette_wheel_selection(population)
    alpha_children = t_children + r_children
    for i in range(0, int(len(alpha_children)/2)):
        first_parent = alpha_children[i]
        second_parent = alpha_children[-(i+1)]
        results.append(first_parent.generate_children(second_parent))
        results.append(second_parent.generate_children(first_parent))
...
def roulette_wheel_selection(population):
    alpha_children = []
    total_fitness = sum([child.fitness() for child in population])
    relative_fitness = [child.fitness()/total_fitness for child in population]
    # print(relative_fitness)
    probability = [sum(relative_fitness[:(i+1)])
                   for i in range(int(len(relative_fitness)))]
    for i in range(int(len(population))):
        randomNum = random.uniform(0, total_fitness)
        for j in range(int(len(population))):
            if randomNum <= probability[i]:
                alpha_children.append(population[i])
                break

    return alpha_children
...
def tournament_selection(population):
    ...
    for i in range(0, int(len(population)/2)):
        ...
        if a.fitness() > b.fitness():
            better_children.append(a)
        else:
            better_children.append(b)
    return better_children
